...
export async function POST(request: Request) {
  try {
    ...
    const payload = {
      ...
      criado_por: normalizarTexto(body.criado_por),
      criado_por_nome: normalizarTexto(body.criado_por_nome) ?? "Sistema",
      atualizado_por: normalizarTexto(body.atualizado_por),
      atualizado_por_nome: normalizarTexto(body.atualizado_por_nome),
    };
    ...
    return NextResponse.json(
      {
        error: "Falha ao processar criação da proposta.",
        details: message,
      },
      { status: 500 }
    );
