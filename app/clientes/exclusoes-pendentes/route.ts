...
export async function GET(req: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Sem permissão para visualizar aprovações de exclusão.",
        },
        { status: 403 }
      );
    }
    ...
    let query = supabaseAdmin
      ...
      .select(
        "id, cliente_id, corretora_id, solicitado_por, cliente_nome, corretora_nome, solicitado_por_nome, motivo, tipo_exclusao, status, solicitado_em"
      )
      .eq("status", "pendente")
    ...
    if (error) {
      return NextResponse.json(
        {
          success: false,
          message: "Erro ao carregar solicitações pendentes.",
    ...
    console.error("Erro interno ao listar exclusões pendentes:", error);

    return NextResponse.json(
      {
        success: false,
        message: "Erro interno ao carregar solicitações pendentes.",
      },
      { status: 500 }
    );
  }
